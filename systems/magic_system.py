...
from gameplay.jrpg_combat import (
    MagicPoints, JRPGStats, SpellList, Spell,
    ElementalResistances, ElementType, TargetType
)
...
class MagicSystem(System):
    ...
    def cast_spell(self, world: World, caster: Entity, spell_id: str,
                   targets: List[Entity]) -> bool:
        """
        ...
        if not mp.consume_mp(spell.mp_cost):
            self.event_manager.emit(Event(
                EventType.CUSTOM,
                {"type": "spell_failed", "reason": "not_enough_mp", "caster_id": caster.id}
            ))
            return False
    ...
    def _apply_spell_effect(self, caster: Entity, target: Entity,
                           spell: Spell, caster_stats: Optional[JRPGStats]):
