...
class SettingsUI:
    ...
    def __init__(self, screen: pygame.Surface, audio_manager: Optional[AudioManager] = None,
    ...
    def render(self):
        ...
        # Title
        self.ui.title("Settings", panel_x + panel_width // 2 - 50, panel_y + 10, size=28, color=(255, 200, 0))
    ...
    def _render_audio_settings(self, x: int, y: int, width: int, height: int):
        ...
        if self.ui.slider(x + 20, current_y, width - 40, 30, self.settings['audio']['master_volume']):
        ...
        self.ui.label(f"{int(self.settings['audio']['master_volume'] * 100)}%",
                     x + width - 60, current_y + 5, size=16)
        current_y += 50
        ...
        if self.ui.slider(x + 20, current_y, width - 40, 30, self.settings['audio']['music_volume']):
            mouse_x = pygame.mouse.get_pos()[0]
            slider_x = mouse_x - x - 20
        ...
        self.ui.label(f"{int(self.settings['audio']['music_volume'] * 100)}%",
                     x + width - 60, current_y + 5, size=16)
        current_y += 50
        ...
        self.ui.label(f"{int(self.settings['audio']['sfx_volume'] * 100)}%",
                     x + width - 60, current_y + 5, size=16)
        current_y += 50
    ...
    def _render_graphics_settings(self, x: int, y: int, width: int, height: int):
        ...
        # Fullscreen toggle
        fullscreen_text = "Fullscreen: ON" if self.settings['graphics']['fullscreen'] else "Fullscreen: OFF"
        ...
        if self.ui.slider(x + 20, current_y, width - 40, 30, self.settings['graphics']['particle_density']):
            mouse_x = pygame.mouse.get_pos()[0]
            slider_x = mouse_x - x - 20
        ...
        # Screen Shake toggle
        shake_text = "Screen Shake: ON" if self.settings['graphics']['screen_shake'] else "Screen Shake: OFF"
        ...
        if self.ui.button(shake_text, x + 20, current_y, 300, 40, color=shake_color):
            self.settings['graphics']['screen_shake'] = not self.settings['graphics']['screen_shake']
            self.has_unsaved_changes = True
    ...
    def _render_input_settings(self, x: int, y: int, width: int, height: int):
        ...
        if self.ui.slider(x + 20, current_y, width - 40, 30, (self.settings['input']['mouse_sensitivity'] - 0.5) / 1.5):
            mouse_x = pygame.mouse.get_pos()[0]
            slider_x = mouse_x - x - 20
        ...
        self.ui.label(f"{self.settings['input']['mouse_sensitivity']:.2f}x",
                     x + width - 80, current_y + 5, size=16)
        current_y += 50

        # Invert Y axis
        invert_text = "Invert Y-Axis: ON" if self.settings['input']['invert_y'] else "Invert Y-Axis: OFF"
    ...
    def _render_gameplay_settings(self, x: int, y: int, width: int, height: int):
        ...
        for difficulty in difficulties:
            ...
            if self.ui.button(difficulty.capitalize(), x + 30, current_y, 150, 35, color=diff_color):
        ...
        # Auto-save
        auto_save_text = "Auto-Save: ON" if self.settings['gameplay']['auto_save'] else "Auto-Save: OFF"
        ...
        # Tutorial
        tutorial_text = "Tutorial: ON" if self.settings['gameplay']['tutorial_enabled'] else "Tutorial: OFF"
        tutorial_color = (0, 150, 0) if self.settings['gameplay']['tutorial_enabled'] else (150, 0, 0)

        if self.ui.button(tutorial_text, x + 20, current_y, 300, 40, color=tutorial_color):
            self.settings['gameplay']['tutorial_enabled'] = not self.settings['gameplay']['tutorial_enabled']
            self.has_unsaved_changes = True
    ...
    def _render_bottom_buttons(self, x: int, y: int, width: int):
        ...
        # Apply button
        if self.ui.button("Apply", x + 20 + button_width + button_spacing, y, button_width, 40,
                         color=(0, 100, 200)):
            self.apply_settings()

        # Cancel button
        if self.ui.button("Cancel", x + 20 + (button_width + button_spacing) * 2, y, button_width, 40,
                         color=(150, 0, 0)):
            self.toggle()

        # Reset to defaults
        if self.ui.button("Reset to Defaults", x + width - button_width - 20, y, button_width, 40,
                         color=(150, 100, 0)):
    ...
    def load_settings(self):
        ...
        try:
            with open('settings.json', 'r') as f:
                ...
                self.key_bindings = {k: int(v) for k, v in settings_data.get('key_bindings', {}).items()}
