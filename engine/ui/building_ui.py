...
class BuildingUI:
    ...
    def _render_building_catalog(self):
        ...
        # Resource display
        resources_y = 50
        self.ui.label("Available Resources:", panel_x + 10, resources_y, size=16, color=(200, 200, 255))
        resources_y += 25
        ...
        for resource, amount in sorted(total_resources.items()):
            self.ui.label(f"{resource.capitalize()}: {amount:.0f}",
                         panel_x + 15, resources_y, size=14)
            resources_y += 20
        ...
        for building_type, definition in self.building_definitions.items():
            # Building button
            button_height = 100
            button_color = (40, 40, 60) if building_type != self.selected_building_type else (60, 60, 100)

            self.ui.panel(panel_x + 10, building_y, panel_width - 20, button_height, button_color)

            # Building name and color swatch
            pygame.draw.rect(self.screen, definition['color'],
                           (panel_x + 15, building_y + 5, 20, 20))
            self.ui.label(definition['name'], panel_x + 40, building_y + 5, size=18, color=(255, 255, 255))

            # Description
            self.ui.label(definition['description'], panel_x + 15, building_y + 30, size=12, color=(200, 200, 200))
            ...
            if definition['production']:
                prod_text = "Prod: " + ", ".join([f"{k}: +{v}" for k, v in definition['production'].items()])
                self.ui.label(prod_text, panel_x + 15, building_y + 65, size=11, color=(100, 255, 100))

            if definition['consumption']:
                cons_text = "Uses: " + ", ".join([f"{k}: -{v}" for k, v in definition['consumption'].items()])
                self.ui.label(cons_text, panel_x + 15, building_y + 80, size=11, color=(255, 200, 100))

            # Click to select
            mouse_pos = pygame.mouse.get_pos()
            if (panel_x + 10 <= mouse_pos[0] <= panel_x + panel_width - 10 and
                building_y <= mouse_pos[1] <= building_y + button_height):
    ...
    def _render_placement_preview(self, camera_offset: Tuple[int, int]):
        ...
        preview_surface = pygame.Surface((size_w * self.tile_size, size_h * self.tile_size), pygame.SRCALPHA)
        ...
        pygame.draw.rect(self.screen, border_color,
                        (screen_x, screen_y, size_w * self.tile_size, size_h * self.tile_size), 3)

        # Draw building name
        font = pygame.font.Font(None, 24)
        text = font.render(definition['name'], True, (255, 255, 255))
        text_rect = text.get_rect(center=(screen_x + size_w * self.tile_size // 2,
                                          screen_y - 15))
    ...
    def _is_valid_placement(self, grid_x: int, grid_y: int, size_w: int, size_h: int) -> bool:
        ...
        if (grid_x < 0 or grid_y < 0 or
            grid_x + size_w > self.grid_width or
            grid_y + size_h > self.grid_height):
            return False
        ...
        for entity_id in self.world.get_entities_with_tag('building'):
            ...
            if grid_pos:
                ...
                # Check overlap
                if (grid_x < grid_pos.grid_x + existing_size[0] and
                    grid_x + size_w > grid_pos.grid_x and
                    grid_y < grid_pos.grid_y + existing_size[1] and
                    grid_y + size_h > grid_pos.grid_y):
                    return False
